import { isType, strToEl, runCallback, hasClass, extend, translit} from './lib/utils.js';
...
class Dropdown {
	...
	*/
	 ...
	 */
	_render() {
		const { target, container, head, list, selectedList, input, hidden, arrow } = this.elements;
	...
	_renderMessage(message, isClean = true) {
		const { list } = this.elements,
					element = this._getTemplate('message', message);
	...
	_onClick(e) {
		const { container, list } = this.elements,
					{ classNames } = this.config,
					isDropdown = container.contains(e.target),
					isActive = list.classList.contains(classNames.listActive);
		
		if(isDropdown) {
			let target = e.target,
			item = target.closest(`.${classNames.listItem}`);
	...
	_scrollToElem(element, dir) {
		const { list } = this.elements,
					...
					itemBottom = element.offsetTop + element.offsetHeight,
					scrollTop = this.elements.list.scrollTop,
					listHeight = this.elements.list.offsetHeight;
